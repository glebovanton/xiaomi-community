...
def info(cookie):
    ...
    print('Nickname: '+result['entity']['userInfo']['userName']+' Level: '+result['entity']['userInfo']['userGrowLevelInfo']['showLevel']+' Points: '+str(result['entity']['userInfo']['userGrowLevelInfo']['point']))
...
def main(accountt=[],password=[]):
    ...
    print('By: CMDQ github@CMDQ8575')
    for i in range(len(account)):
        cookie = login(account[i],password[i])
        if cookie:
            for action in ['info','check_in','like','browse','carrot','check_status']:
                name = {'info':'User Info','check_in':'Check-in','like':'Like','browse':'Browse','carrot':'Carrot','check_status':'Check-in Status'}
                try:
                    eval(f'{action}(cookie)')
                except Exception as e:
                    print('Task ' + name[action] + ' failed',e)
        else:
            print(f'{account[i]}: Login failed')

if __name__ == '__main__':
    os.chdir(os.path.dirname(__file__)) # Comment out for Pyinstaller
    if os.path.exists('config.toml'):
        ...
        print('First run, config file generated: ' + os.path.join(os.getcwd(),'config.toml'))
